  ...
  <div>
    ...
    >
      <div
        class="lct_body"
        v-bind:style="{
          width: FC.BaseMap_TSCJZ.w + 'px',
        }"
      >
        <div class="btmimg">
          <img
            v-bind:style="{
              left: FC.BaseMap_TSCJZ.x + 'px',
              top: FC.BaseMap_TSCJZ.y + 'px',
              'z-index': FC.BaseMap_TSCJZ.zindex,
            }"
            v-bind:src="require('@img/' + FC.BaseMap_TSCJZ.img)"
            v-show="FC.BaseMap_TSCJZ.visible"
          />
        ...
        />
          ...
          >
            <div
              ...
              @click="
                sendbfml(
                  'VOCs采样杯排水阀19',
                  SDL_Global.CMD.PLC_SV.Format(19, !SDL_Global.PLC.SV19Status)
                )
              "
            ></div>
          </el-tooltip>
          ...
          >
            <div
              ...
              @click="
                sendbfml(
                  'VOCs采样杯采水阀20',
                  SDL_Global.CMD.PLC_SV.Format(20, !SDL_Global.PLC.SV20Status)
                )
              "
            ></div>
          </el-tooltip>
          ...
          >
            <div
              ...
              @click="
                sendbfml(
                  'VOCs采样杯清洗阀22',
                  SDL_Global.CMD.PLC_SV.Format(22, !SDL_Global.PLC.SV22Status)
                )
              "
            ></div>
          </el-tooltip>
          ...
          <div class="vocsclass">
            <div class="ybData">
              {{ SDL_Global.VOCs.ErLvYiWan12.Name }}
            </div>

            <!--v-model="Is_TSCJZ" :manual='true'  -->
            <el-tooltip
              class="item"
              effect="dark"
            ...
            >
              <div slot="content">
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.JiaBen.Name }}:
                  {{ SDL_Global.VOCs.JiaBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.LvBen.Name }}:
                  {{ SDL_Global.VOCs.LvBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.ErLvBen12.Name }}:
                  {{ SDL_Global.VOCs.ErLvBen12.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.ErLvBen14.Name }}:
                  {{ SDL_Global.VOCs.ErLvBen14.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.YiBingBen.Name }}:
                  {{ SDL_Global.VOCs.YiBingBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.BenYiXi.Name }}:
                  {{ SDL_Global.VOCs.BenYiXi.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.DuiWenErJiaBen.Name }}:
                  {{ SDL_Global.VOCs.DuiWenErJiaBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.SanXiuJiaWan.Name }}:
                  {{ SDL_Global.VOCs.SanXiuJiaWan.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.SanLvJiaWan.Name }}:
                  {{ SDL_Global.VOCs.SanLvJiaWan.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.LinErJiaBen.Name }}:
                  {{ SDL_Global.VOCs.LinErJiaBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.YiErLvYiXi11.Name }}:
                  {{ SDL_Global.VOCs.YiErLvYiXi11.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.YiErLvYiXi12.Name }}:
                  {{ SDL_Global.VOCs.YiErLvYiXi12.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.YiBen.Name }}:
                  {{ SDL_Global.VOCs.YiBen.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.ErLvJiaWan.Name }}:
                  {{ SDL_Global.VOCs.ErLvJiaWan.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.SanLvYiXi.Name }}:
                  {{ SDL_Global.VOCs.SanLvYiXi.LastTestData }}
                </div>
                <div style="margin-bottom: 3px">
                  {{ SDL_Global.VOCs.SiLvYiXi.Name }}:
                  {{ SDL_Global.VOCs.SiLvYiXi.LastTestData }}
                </div>
                <div>
                  {{ SDL_Global.VOCs.Ben.Name }}:
                  {{ SDL_Global.VOCs.Ben.LastTestData }}
                </div>
              </div>
              <div class="ybVal">
                <div>{{ SDL_Global.VOCs.ErLvYiWan12.LastTestData }}</div>
              </div>
            </el-tooltip>
            <div>
              <i
                style="position: relative; left: 43px; top: -3px"
                class="el-icon-caret-bottom"
              ></i>
            </div>

            <div class="ybUnit">
              {{ SDL_Global.VOCs.ErLvYiWan12.Unit }}
            </div>

            <div class="ybState">
              <span
                v-if="SDL_Global.VOCs.ComStatus != 1"
                style="
                  width: 10px;
                  height: 10px;
                  background-color: #cccccc;
              ...
              <span
                ...
                "
              ></span>
              {{ SDL_Global.VOCs.MeterStatusText }}
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    dialoglctVisible: {
      // getter
      get: function () {
        return this.TSlctmodel;
      },
      // setter
      set: function (newVal) {
        if (!newVal) {
          this.$emit("update:TSlctmodel", false); //触发 input 事件，并传入新值
        }
      },
    },
  },
