                        ...
                        <el-row>
                            <el-col :sm="12" :md="12" :lg="9" :xl="6">
                                ...
                                <el-date-picker v-model="dateValue" type="datetimerange" unlink-panels
                                    ...
                                    class="times" value-format="yyyy-MM-dd" size="small" @change="inquire">
                                </el-date-picker>
                            </el-col>
                        </el-row>
    ...
    export default {
        ...
        methods: {
            inquire() {
                ...
                this.xAxisData = []; //x轴数据
                this.seriesData = []; //仪表备品使用数据源
                this.seriesResizeData = []; //但仪表备品使用详情数据源
                let condition = "";
                condition =
                    "  AddTime BETWEEN '" +
                    this.dateValue[0] +
                    ...
                    this.dateValue[1] +
                    "'group by a.`PolId`";
                let tabsource = "tbsparepart a RIGHT JOIN tbsparepartuse b on a.ID=b.SparePartID"


                let queryCol = "`PolId`,sum(b.UseCount) ";
                //获取列名
                this.http
                    .comMethod(queryCol, tabsource, condition)
                    .then((res) => {
                        // setTimeout(() => {
                        this.pictLoading = false;
                        // }, 1000);
                        if (res.length != 0) {
                            ...
                            for (let i = 0; i < res.length; i++) {
                                ...
                                factor =this.socketApi.globalDataConfig.getCodeNameData(res[i].PolId)// this.funfactor(res[i].PolId)
                                this.xAxisData.push(factor)

                                let obj = {
                                    value: res[i]["sum(b.UseCount)"],
                                    groupId: factor,
                                };
                                this.seriesData.push(obj);
                            }

                            // console.log('x轴', this.xAxisData)
                            // console.log('新serie', this.seriesData)
                            let conditions = "  AddTime BETWEEN '" +
                                this.dateValue[0] +
                                "' and '" +
                                this.dateValue[1] +
                                "' ";

                            let queryCols = " b.ID, a.`Name`,b.UseCount ,a.PolId"
                            //获取详情
                            this.http
                                .comMethod(queryCols, tabsource, conditions)
                                .then((res) => {

                                    if (res.length != 0) {
                                        // var datatests = [];
                                        console.log('备品备件', res)
                                        let $this = this;
                                        // let factors = ""
                                        // var arrdata = []
            ...
            echart: function () {
                ...
                this.factorOption = {
                    ...
                    tooltip: {
                        ...
                        formatter(params) {
                            ...
                            return params[0].name + '<br/>' + '备件更换个数: ' + params[0].value;
                ...
                $this.myChart.on('click', function (event) {
                    if (event.data) {
                        ...
                        let alarmOptions = {
                            title: {
                                text: subData.dataGroupId + '-备件更换统计',
                                textStyle: {
                                    color: '#fff'
                                },
