...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      //alert(moment(that.dateValue[1]).diff(moment(that.dateValue[0]), "day"));
      that.myChart4Option = {
        color: ["#67C23A", "#E6A23C"],
        title: [
          {
        ...
        legend: {
          ...
          data: [
            "运行时长（注：指仪表周期测量产生的时长）",
            "无效时长（注：指仪表周期测量日质控标样核查未合格产生的时长）",
            "维护时长（注：指仪表维护和待机产生的时长）",
          ],
          textStyle: {
            color: "#fff",
          },
        ...
        series: [
          ...
          //     center: ["20%", "30%"],
          //     data: [
          //       { value: 34, name: "运行时长(小时)" },
          //       { value: 2, name: "故障时长(小时)" },
          //       { value: 12, name: "维护时长(小时)" },
          //     ],
          //     label: {
          //       normal: {
          //         show: true,
          ...
          //     center: ["50%", "30%"],
          //     data: [
          //       { value: 22, name: "运行时长(小时)" },
          //       { value: 12, name: "故障时长(小时)" },
          //       { value: 14, name: "维护时长(小时)" },
          //     ],
          //     label: {
          //       normal: {
          //         show: true,
      ...
      that.myChart4.clear();
      that.myChart4.setOption(that.myChart4Option);
      this.getChartData(function (res) {
        ...
        if (res && res.length > 0) {
          ...
          data.forEach(function (item, index) {
            ...
            // data.PollutantName = that.SDL_Global.filter(
            //   (m) => m.Code == item.PollutantCode
            // )[0].Name;
            ...
            that.myChart4Option.title.push({
            ...
            that.myChart4Option.series.push({
              ...
              data: [
                {
                  value: item.EffectiveNum * 24,
                  name: "运行时长（注：指仪表周期测量产生的时长）",
                },
                {
                  value: item.InvalidNum * 24,
                  name: "无效时长（注：指仪表周期测量日质控标样核查未合格产生的时长）",
                },
                {
                  value:
                    (moment(that.dateValue[1]).diff(
                      moment(that.dateValue[0]),
                      "day"
                    ) -
                      item.EffectiveNum -
                      item.InvalidNum) *
                    24,
                  name: "维护时长（注：指仪表维护和待机产生的时长）",
                },
              ],
              label: {
                normal: {
                  ...
                  formatter: function (a1, a2, a3) {
                    //debugger
                    //return "{c}(h)";
                    if (a1.data.name.indexOf("运行时长") > -1) {
                      return a1.percent + "%";
                    } else {
                      return a1.data.value + "(h)";
                    }
                    return a1.data.name + "(h)";
                  },
                },
              },
            });
      ...
      //       that.myChart4Option.xAxis.data = that.xAxisData;
      //       that.myChart4Option.series.data = that.seriesData;
      //       that.myChart4.setOption(that.myChart4Option);
      //       if (firstCode != "") {
      //         that.getTableData(firstCode);
      //       }
      ...
      //   });
      that.pictLoading = false;
      that.myChart4.getZr().off("click");

      //点击柱状图，查看详情
      that.myChart4.getZr().on("click", function (event) {
        // 解决数值太小，点击困难的问题
        let point = [event.offsetX, event.offsetY];
        if (that.myChart4.containPixel("grid", point)) {
          let xIndex = that.myChart4.convertFromPixel(
            { seriesIndex: 0 },
            point
          )[0];
          let op = that.myChart4.getOption();
